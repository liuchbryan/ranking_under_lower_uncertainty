...
def get_samples(
        ...
        sigma_sq_X: float, sigma_sq_1: float, sigma_sq_2: float, nu: float,
        verbose=True):
    ...
    for i in range(0, n_samples):
        # Step 1
        propositions = (
            pd.DataFrame(mu_X + np.random.standard_t(nu, N) * sigma_X * np.sqrt((nu - 2) / nu),
                         columns=['true']))

        # Step 2
        propositions['observed_noisy'] = (
                propositions['true'] +
                mu_epsilon + np.random.standard_t(nu, N) * sigma_1 * np.sqrt((nu - 2) / nu))
        ...
        propositions['observed_clean'] = (
                propositions['true'] +
                mu_epsilon + np.random.standard_t(nu, N) * sigma_2 * np.sqrt((nu - 2) / nu))
